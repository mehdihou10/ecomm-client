...
const PendingVendors = () => {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch(`${url}/api/admin/accounts/vendor/pending`, {
        headers: {
          Authorization: `Bearer ${cookies.user}`,
        },
      });
      ...
      if (responseData.status === "fail") {
        ...
        setVendors(responseData.vendorsAccounts);
      }
    } catch (error) {
    ...
    <>
      ...
      <div className="sm:flex">
        ...
        <div className="flex-1">
          ...
          <hr />
          {vendors.length === 0 ? (
            <h1 className="p-4">No pending vendors</h1>
          ) : (
            <div className="">
                        ...
                        <td className=" p-3 text-sm text-gray-700">
                          <button>
                            <AiOutlineCheckCircle
                              onClick={() => acceptVendor(vendor.id)}
